...
function isSafeIdentifier(string) {
  return /^[a-z_$][\w$]*$/i.test(string);
}
function toSafeIdentifier(string) {
  if (isSafeIdentifier(string))
    return string;
  return normalizeIdentifier(string);
}
function normalizeIdentifier(string) {
  return upperFirst$1(
    // remove accents, umlauts, ... by their basic latin letters
    deburr(string).replace(/(^\s*[^a-z_$])|([^\w$])/gi, " ").replace(/^_[a-z]/g, (match) => match.toUpperCase()).replace(/_[a-z]/g, (match) => match.substr(1, match.length).toUpperCase()).replace(/([\d$]+[a-z])/gi, (match) => match.toUpperCase()).replace(/\s+([a-z])/gi, (match) => trim(match.toUpperCase())).replace(/\s/g, "")
  );
}
...
` : ""}export ${options.enableConstEnums ? "const " : ""}enum ${toSafeIdentifier(ast.standaloneName)} {
${ast.params.map(({ ast: ast2, keyName }) => `${keyName} = ${generateType(ast2, options)}`).join(",\n")}
}`;
}
function generateStandaloneInterface(ast, options) {
  return `${hasComment(ast) ? `${generateComment(ast.comment, ast.deprecated)}
` : ""}export interface ${toSafeIdentifier(ast.standaloneName)} ${ast.superTypes.length > 0 ? `extends ${ast.superTypes.map((superType) => toSafeIdentifier(superType.standaloneName)).join(", ")} ` : ""}${generateInterface(ast, options)}`;
}
